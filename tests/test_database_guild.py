...
class TestGuildTableCreation:
    ...
    def test_guild_table_creation(self, temp_db):
        ...
        assert (
            "CHECK(faction IN ('Alliance', 'Horde'))" in guild_schema
        ), "Guild table should have faction constraint"

        # Check unique constraint
        assert (
            "UNIQUE(guild_name, server, region)" in guild_schema
    ...
    def test_guild_id_column_exists_in_all_tables(self, temp_db):
        ...
        tables_with_guild_id = ["encounters", "characters", "character_metrics", "log_files"]
        ...
        for table_name in tables_with_guild_id:
            ...
            if cursor.fetchone():  # Table exists
                ...
                assert "guild_id" in columns, f"Table {table_name} should have guild_id column"
...
class TestGuildMigration:
    ...
    def test_migration_idempotency(self, temp_db):
        ...
        assert final_guild_count == initial_guild_count, "Migration should not duplicate guilds"
        ...
        assert final_version == initial_version, "Schema version should remain unchanged"
...
class TestGuildDataIsolation:
    ...
    def test_guild_encounter_isolation(self, multi_guild_setup):
        ...
        assert set(guild4_encounters) == {"Boss D"}, "Guild 4 should only see its own encounters"
...
class TestGuildIndexPerformance:
    ...
    def test_guild_index_creation(self, temp_db):
        ...
        for expected_index in expected_indexes:
            assert expected_index in actual_indexes, f"Index {expected_index} should exist"
...
class TestGuildDataIntegrity:
    ...
    def test_guild_foreign_key_integrity(self, temp_db):
        ...
        assert len(violations) == 0, f"No foreign key violations should exist, found: {violations}"
