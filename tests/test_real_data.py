...
class TestRealDataParser:
    ...
    def test_parser_all_example_files(self, example_files):
        ...
        for log_file in example_files:
            ...
            assert success_rate > 0.8, f"Parser success rate should be >80% for {log_file.name}"
    ...
    def test_compression_with_real_data(self, small_log_file):
        ...
        if events:
            ...
            assert 0.1 <= compression_ratio <= 1.0, "Compression ratio should be reasonable"

            print(f"Compressed {len(events)} events with ratio: {compression_ratio:.3f}")
    ...
    def test_performance_benchmarks(self, example_files):
        ...
        for benchmark in benchmarks:
            assert benchmark['lines_per_second'] > 100, f"Performance too slow for {benchmark['file']}"
    ...
    def test_memory_usage(self, small_log_file):
        ...
        with open(small_log_file, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                ...
                if i > 0 and i % 1000 == 0:
                    ...
                    assert memory_growth < 100, f"Memory usage growing too fast: {memory_growth:.1f}MB"
        ...
        if len(events) > 0:
            ...
            assert memory_per_event < 10, f"Memory per event too high: {memory_per_event:.2f}KB"
    ...
    def test_character_stream_extraction(self, small_log_file):
        ...
        if character_events:
            ...
            most_active_char = max(character_events.keys(), key=lambda x: len(character_events[x]))
            event_count = len(character_events[most_active_char])

            assert event_count > 0, "Should find events for characters"
            print(f"Most active character: {most_active_char} with {event_count} events")
    ...
    def test_combat_log_format_validation(self):
        ...
        for log_file in files[:2]:  # Test first 2 files
            with open(log_file, 'r', encoding='utf-8') as f:
                ...
                assert 'COMBAT_LOG_VERSION' in first_line, f"Invalid combat log format in {log_file.name}"

                # Should have timestamp format
                assert '/' in first_line and ':' in first_line, f"Missing timestamp in {log_file.name}"
            ...
            with open(log_file, 'r', encoding='utf-8') as f:
                for i, line in enumerate(f):
                    ...
                    if 'ENCOUNTER_' in line or 'CHALLENGE_MODE_' in line:
                        ...
                        print(f"Found encounter event in {log_file.name}: {line[:100]}...")
                        break
