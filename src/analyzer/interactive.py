...
class InteractiveAnalyzer:
    ...
    def _convert_encounters_to_fights(self, encounters: List[UnifiedEncounter]) -> List[Fight]:
        ...
        for enc in encounters:
            ...
            fight = Fight(
                ...
                combat_duration=(
                    enc.combat_duration if hasattr(enc, "combat_duration") else enc.duration
                ),
                ...
                metadata={
                    ...
                    "metrics": (
                        enc.metrics.to_dict() if hasattr(enc, "metrics") and enc.metrics else {}
                    ),
                },
            )
            ...
            if enc.encounter_type == EncounterType.MYTHIC_PLUS and enc.fights:
                for internal_fight in enc.fights:
                    sub_fight_type = (
                        FightType.DUNGEON_BOSS if internal_fight.is_boss else FightType.TRASH
                    )
                    ...
                    sub_fight = Fight(
                        ...
                        keystone_level=enc.keystone_level,
                        success=(
                            internal_fight.success if hasattr(internal_fight, "success") else None
                        ),
                        duration=internal_fight.duration,
                        combat_duration=(
                            internal_fight.combat_time
                            if hasattr(internal_fight, "combat_time")
                            else internal_fight.duration
                        ),
                        combat_periods=(
                            internal_fight.combat_periods
                            if hasattr(internal_fight, "combat_periods")
                            else []
                        ),
                        ...
                        for guid, char in internal_fight.players.items():
                            sub_fight.participants[guid] = {
                                ...
                                "class": (
                                    ...
                                    if hasattr(char, "character_class")
                                    else None
                                ),
                                "spec": char.spec if hasattr(char, "spec") else None,
                                "damage_done": (
                                    char.damage_done if hasattr(char, "damage_done") else 0
                                ),
                                "healing_done": (
                                    char.healing_done if hasattr(char, "healing_done") else 0
                                ),
                                "damage_taken": (
                                    char.damage_taken if hasattr(char, "damage_taken") else 0
                                ),
                                "is_player": guid.startswith("Player-") if guid else False,
                                "is_pet": guid.startswith("Pet-") if guid else False,
                            }
    ...
    def _perform_search(self, query: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        # Search encounters - use the converted fights for consistency
        for fight in self.fights:
