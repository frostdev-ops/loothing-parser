...
class MetricsCalculator:
    ...
    @staticmethod
    def calculate_encounter_metrics(fight: Fight, characters: Dict[str, CharacterEventStream]) -> Dict[str, Any]:
        ...
        # Calculate raid-wide DPS/HPS
        if fight.duration > 0:
            metrics["raid_dps"] = metrics["total_damage"] / fight.duration
            metrics["raid_hps"] = metrics["total_healing"] / fight.duration

        # Calculate average activity
        active_chars = [char for char in characters.values() if char.activity_percentage > 0]
        if active_chars:
            metrics["activity_avg"] = sum(char.activity_percentage for char in active_chars) / len(active_chars)

        return metrics

    @staticmethod
    def get_dps_rankings(characters: Dict[str, CharacterEventStream], duration: float) -> List[Tuple[str, float, CharacterEventStream]]:
    ...
    @staticmethod
    def get_hps_rankings(characters: Dict[str, CharacterEventStream], duration: float) -> List[Tuple[str, float, CharacterEventStream]]:
    ...
    def get_survival_stats(characters: Dict[str, CharacterEventStream]) -> Dict[str, Any]:
        ...
        return {
            ...
            "survival_rate": (total_players - len(deaths_by_player)) / total_players if total_players > 0 else 0,
    ...
    def analyze_performance_trends(fights: List[Fight]) -> Dict[str, Any]:
        ...
        if successful_fights:
            ...
            if valid_success:
                trends["avg_duration_success"] = sum(f.duration for f in valid_success) / len(valid_success)

        if failed_fights:
            valid_failed = [f for f in failed_fights if f.duration]
            if valid_failed:
                trends["avg_duration_wipe"] = sum(f.duration for f in valid_failed) / len(valid_failed)
        ...
        for fight in fights:
            if fight.difficulty:
                ...
                trends["difficulty_distribution"][diff] = trends["difficulty_distribution"].get(diff, 0) + 1

        return trends

    @staticmethod
    def calculate_fight_efficiency(fight: Fight, characters: Dict[str, CharacterEventStream]) -> Dict[str, float]:
        ...
        active_chars = [char for char in characters.values() if char.activity_percentage > 0]
        if active_chars:
            efficiency["avg_activity"] = sum(char.activity_percentage for char in active_chars) / len(active_chars)

        # Survival efficiency
        total_players = len(characters)
        deaths = sum(char.death_count for char in characters.values())
        efficiency["survival_rate"] = (total_players * 1.0 - deaths) / total_players if total_players > 0 else 0

        return efficiency

    @staticmethod
    def get_top_abilities(characters: Dict[str, CharacterEventStream], ability_type: str = "damage") -> List[Tuple[str, str, int]]:
