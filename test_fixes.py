...
from src.segmentation.enhanced import EnhancedEncounterSegmenter
...
def test_parser_fixes():
    ...
    floodgate_runs = [run for run in results['mythic_plus_runs']
                     if 'Floodgate' in run.dungeon_name]
    ...
    print(f"\nTop 5 DPS:")
    dps_chars = sorted(run.overall_characters.values(),
                      key=lambda x: x.total_damage_done, reverse=True)[:5]
    ...
    print(f"\nTop 3 Healers:")
    heal_chars = sorted(run.overall_characters.values(),
                       key=lambda x: x.total_healing_done, reverse=True)[:3]

    for i, char in enumerate(heal_chars, 1):
        hps = char.get_combat_hps() if char.combat_time > 0 else char.get_hps(run.actual_time_seconds)
        print(f"  {i}. {char.character_name}: {char.total_healing_done:,} healing, "
